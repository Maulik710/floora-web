...
function transformProduct(entry) {
  return {
    ...
    colorHex: entry.color?.hex ?? "#CCCCCC",
    size: entry.size,
...
const PRODUCT_POPULATE =
  "populate[category]=true&populate[color]=true&populate[rooms]=true&populate[images]=true&populate[specs]=true";
...
export async function getColors() {
  const data = await strapiFetch("/colors?sort=name:asc&pagination[pageSize]=100");
  return data?.map((c) => ({ name: c.name, hex: c.hex })) ?? null;
}
