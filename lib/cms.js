...
// failure — network error, CMS down, empty collection.
...
function transformProduct(entry) {
  return {
    ...
    categoryName: entry.category?.short ?? entry.category?.name ?? "",
...
function transformHomePage(entry) {
  return {
    ...
    clientJourney: (entry.clientJourney ?? []).map((j) => ({
      icon: j.icon,
      title: j.title,
...
function transformAboutPage(entry) {
  return {
    ...
    heroImage: mediaUrl(entry.heroImage),
    storyEyebrow: entry.storyEyebrow,
    storyHeading: entry.storyHeading,
    storyBody: entry.storyBody,
    storyImage: mediaUrl(entry.storyImage),
    storyBullets: (entry.storyBullets ?? []).map((b) => b.text),
    missionQuote: entry.missionQuote,
    manufacturingImages: (entry.manufacturingImages ?? []).map((m) => ({
      image: mediaUrl(m.image),
      label: m.label,
    })),
    whyChooseUs: (entry.whyChooseUs ?? []).map((w) => ({ icon: w.icon, title: w.title, text: w.text })),
...
const HOME_PAGE_POPULATE =
  "populate[heroImage]=true&populate[stats]=true&populate[whyChoose]=true&populate[clientJourney]=true&populate[certifications]=true&populate[inspirationImage]=true&populate[inspirationBullets]=true&populate[ctaImage]=true";

const ABOUT_PAGE_POPULATE =
  "populate[heroImage]=true&populate[storyImage]=true&populate[storyBullets]=true&populate[manufacturingImages][populate][image]=true&populate[whyChooseUs]=true&populate[values]=true&populate[milestones]=true&populate[stats]=true";
...
// Same "related" / "colour variant" selection logic as a static catalogue
// would use, just operating over whatever product list is passed in.
